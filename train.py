...
# FCN 8S  
# step1: 将pool4的结果经过 1*1卷积，channel由 512变为 number_of_classes
pool4_feature = end_points['vgg_16/pool4']
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_16s = slim.conv2d(pool4_feature, number_of_classes, [1, 1], activation_fn=None, padding='SAME',
                                 weights_initializer=tf.zeros_initializer, scope='conv_pool4')

# step2: 将VGG16的输出logits 2倍上采样转置卷积后，与step1的结果 fuse(加和）
upsample_filter_np_x2 = bilinear_upsample_weights(2,number_of_classes)
upsample_filter_tensor_x2 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/transpose_conv1_x2')
logits_x2 = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2, output_shape=tf.shape(aux_logits_16s),
                                          strides=[1, 2, 2, 1], padding='SAME')
fuse_16s = logits_x2 + aux_logits_16s

# step3: 将pool3的结果经过 1*1卷积，channel由 256变为 number_of_classes
pool3_feature = end_points['vgg_16/pool3']
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_8s = slim.conv2d(pool3_feature, number_of_classes, [1, 1], activation_fn=None, padding='SAME',
                                 weights_initializer=tf.zeros_initializer, scope='conv_pool3')
    
# step4: 将 step2的 fuse_16s 经过 2倍上采样转置卷积后，与step3的结果 fuse(加和）    
upsample_filter_np_x2 = bilinear_upsample_weights(2, number_of_classes)  #upsample_factor
upsample_filter_tensor_x2 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/transpose_conv2_x2')
logits_x4 = tf.nn.conv2d_transpose(fuse_16s, upsample_filter_tensor_x2, output_shape=tf.shape(aux_logits_8s),
                                 strides=[1, 2, 2, 1], padding='SAME')
fuse_8s = logits_x4 + aux_logits_8s

# step5: 将 step4的 fuse_8s 经过 8倍上采样转置卷积后，得到最终的 upsampled_logits
...
upsampled_logits = tf.nn.conv2d_transpose(fuse_8s, upsample_filter_tensor_x16, output_shape=upsampled_logits_shape,
